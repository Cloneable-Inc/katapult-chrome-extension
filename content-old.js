...
function createExportButton() {
  ...
  button.addEventListener('click', () => {
    // Go directly to the advanced import interface
...
function showAdvancedImportInterface() {
  ...
  // Load the import interface script if needed, then show modal
  const loadScript = document.createElement('script');
  loadScript.src = chrome.runtime.getURL('import-interface-content.js');
  loadScript.onload = function() {
    console.log('[Cloneable] Import interface loaded');
    this.remove();
    
    // Now create and show the modal using the loaded interface
    if (window.contentImportInterface) {
      // First, update the interface with actual node types if available
      if (window.cloneableNodeTypes && window.cloneableNodeTypes.length > 0) {
        ...
        // Update the node types in the interface
        ...
        const nodeTypeAttr = window.contentImportInterface.availableAttributes.withPicklists.find(attr => attr.name === 'node_type');
        if (nodeTypeAttr) {
      ...
      const modal = window.contentImportInterface.createImportModal();
      document.body.appendChild(modal);
      console.log('[Cloneable] Modal added to page');
    } else {
      console.error('[Cloneable] Import interface not available');
    }
  };
  loadScript.onerror = function(e) {
    console.error('[Cloneable] Failed to load import interface:', e);
  };
