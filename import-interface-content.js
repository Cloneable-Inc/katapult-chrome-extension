...
// This version runs in the content script context

// Only declare if not already defined
if (typeof ImportInterface === 'undefined') {
class ImportInterface {
  ...
  addImageAttachment(modal) {
    const imageMode = modal.querySelector('input[name="image-mode"]:checked').value;
    ...
    modal.querySelector('#image-attachments-list').insertAdjacentHTML('beforeend', attachmentHtml);
    
    // Add event listener for remove button
    const removeBtn = modal.querySelector(`#attachment-${attachmentId} .remove-attachment-btn`);
    removeBtn.addEventListener('click', () => {
      this.removeImageAttachment(attachmentId);
    });
  }

  removeImageAttachment(attachmentId) {
    const attachment = document.getElementById(`attachment-${attachmentId}`);
    if (attachment) {
      attachment.remove();
    }
...
} // End of if (typeof ImportInterface === 'undefined')

// Create global instance only if not already created
if (!window.contentImportInterface) {
  window.contentImportInterface = new ImportInterface();
}